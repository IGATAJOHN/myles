...
function summarizeProduct(product) {
  ...
  return {
    ...
    imageUrl: product.imageUrl || primaryVariant?.imageUrl || null,
    variants: activeVariants.map((variant) => toVariantSummary(variant)),
...
export async function deleteProductVariant(id) {
  ...
  if (variantCount <= 1) {
    throw new Error("You must keep at least one variety for this product.");
  }
...
export async function createInventoryProduct(data) {
  ...
  await prisma.productVariant.create({
    data: {
      productId: product.id,
      label: String(data.variantLabel || "Core").trim() || "Core",
      color: String(data.variantColor || colors[0] || "").trim(),
      price: Math.max(0, Number(data.price) || 0),
      stock: Math.max(0, Number(data.stock) || 0),
      imageUrl: data.imageUrl || null,
      sortOrder: 0
